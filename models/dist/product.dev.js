...
/*#__PURE__*/
function () {
  function Product(productId, title, imageUrl, description, price) {
  ...
  _createClass(Product, [{
    ...
    value: function save() {
      ...
      getProductsFromFile(function (products) {
        if (_this.id) {
          ...
          _this.id = (Math.random() * 1000).toString();
          products.push(_this);
    ...
    value: function deleteById(id) {
      getProductsFromFile(function (products) {
        ...
        if (!product) {
          // If the product is not found, return early or handle the error appropriately
          console.error("Product with id ".concat(id, " not found."));
          return;
        }

        var productPrice = product.price;
        var updatedProduct = products.filter(function (prod) {
          return prod.id !== id;
        });
        fs.writeFile(p, JSON.stringify(updatedProduct), function (err) {
          if (!err) {
